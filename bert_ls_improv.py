...
def data_loader_and_train(train_encodings, valid_encodings, train_labels, valid_labels):
    import torch
    from transformers.file_utils import is_tf_available, is_torch_available, is_torch_tpu_available
    from transformers import BertTokenizerFast, BertForSequenceClassification
    from transformers import Trainer, TrainingArguments
...
def executing_pipeline():
    ...
    train_encodings, valid_encodings, train_labels, valid_labels = pre_process()

    print('#2 data loading')
    final_x=data_loader_and_train(train_encodings, valid_encodings, train_labels, valid_labels)

    # print('#3 Train Step Launching')
    # final_x = train(train_dataset, valid_dataset)
